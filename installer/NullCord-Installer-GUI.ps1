...
<#
    NullCord - graphical installer (WinForms)

    A windowed, on-brand installer (no console window) that downloads the latest NullCord build
    from GitHub Releases and patches the Discord desktop client. Compiled to NullCord-Installer.exe
...
try {
    Add-Type -Namespace NullCordNative -Name Win32 -MemberDefinition @'
...
public class NullCordMenuColors : ProfessionalColorTable {
    public Color Bg = Color.FromArgb(33, 21, 28);
    public Color Sel = Color.FromArgb(72, 45, 60);
    public Color Bd = Color.FromArgb(64, 40, 53);
...
public static class NullCordImaging {
    public static Bitmap HueShift(Image src, float delta) {
        ...
        System.Runtime.InteropServices.Marshal.Copy(d.Scan0, px, 0, bytes);
        for (int i = 0; i < bytes; i += 4) {
            float b = px[i] / 255f;
            float gc = px[i + 1] / 255f;
            float r = px[i + 2] / 255f;
            float max = Math.Max(r, Math.Max(gc, b));
            float min = Math.Min(r, Math.Min(gc, b));
            if (max == min) continue;
            float l = (max + min) / 2f;
            float dlt = max - min;
            float s = l > 0.5f ? dlt / (2f - max - min) : dlt / (max + min);
            if (s < 0.15f) continue;
            float h;
            if (max == r) h = ((gc - b) / dlt + (gc < b ? 6f : 0f)) / 6f;
            else if (max == gc) h = ((b - r) / dlt + 2f) / 6f;
            else h = ((r - gc) / dlt + 4f) / 6f;
            h = h + delta / 360f;
            h = h - (float)Math.Floor(h);
            float q = l < 0.5f ? l * (1f + s) : l + s - l * s;
            float p = 2f * l - q;
            px[i] = (byte)Math.Round(HueToRgb(p, q, h - 1f / 3f) * 255f);
            px[i + 1] = (byte)Math.Round(HueToRgb(p, q, h) * 255f);
            px[i + 2] = (byte)Math.Round(HueToRgb(p, q, h + 1f / 3f) * 255f);
        }
        System.Runtime.InteropServices.Marshal.Copy(px, 0, d.Scan0, bytes);
        bmp.UnlockBits(d);
        return bmp;
    }
...
function Set-DarkTitlebar([IntPtr]$hwnd) {
    ...
    try {
        ...
        [void][NullCordNative.Win32]::DwmSetWindowAttribute($hwnd, 20, [ref]$on, 4)
        [void][NullCordNative.Win32]::DwmSetWindowAttribute($hwnd, 19, [ref]$on, 4)
...
$script:LangNames = @("English", "Deutsch", "Español", "Français", "???????")
...
$script:Strings = @{
    ...
    de = @{
        ...
        msgDoneUninstall = "NullCord entfernt. Starte Discord erneut für einen sauberen Client."
    ...
    es = @{
        ...
        logDownloading   = "Descargando la última versión de NullCord..."
        ...
        noteDone         = "Listo"
        logDoneInstall   = "Listo. NullCord está instalado."
        ...
        msgDoneInstall   = "¡NullCord instalado! Discord se está reiniciando - si no se abre, ábrelo tú."
    ...
    fr = @{
        ...
        logDownloading   = "Téléchargement de la dernière version de NullCord..."
        ...
        noteDone         = "Terminé"
        logDoneInstall   = "Terminé. NullCord est installé."
        ...
        msgDoneInstall   = "NullCord installé ! Discord redémarre - s'il ne se rouvre pas, lancez-le."
        msgDoneUninstall = "NullCord supprimé. Relancez Discord pour un client propre."
...
$script:StringsBlue = @{
    ...
    fr = @{
        headline1        = "Discord,"
        headline2        = "à votre façon"
        tagline          = "Des plugins, des thèmes et un contrôle total - patché dans votre Discord en quelques secondes."
...
function T([string]$key) {
    if ($script:StyleId -eq "blue") {
        $ov = $script:StringsBlue[$script:Lang]
        if ($ov -and $ov.ContainsKey($key)) { return $ov[$key] }
        $ovEn = $script:StringsBlue["en"]
        if ($ovEn -and $ovEn.ContainsKey($key)) { return $ovEn[$key] }
    }
...
function CA([int]$a, $c) { return [System.Drawing.Color]::FromArgb($a, $c.R, $c.G, $c.B) }

$script:Palettes = @{
    pink = @{
        Bg = C3 17 10 14; Panel = C3 33 21 28; Panel2 = C3 48 30 40
        Text = C3 255 255 255; Muted = C3 214 164 192; Faint = C3 141 104 124
        Accent = C3 255 95 166; AccentHi = C3 255 130 188
        RedHi = C3 240 110 134; Border = C3 64 40 53
        ArtTop = C3 58 32 46; ArtBottom = C3 26 14 20
        CheckOff = C3 120 90 108; MenuSel = C3 72 45 60
    }
    blue = @{
        Bg = C3 10 13 18; Panel = C3 21 27 35; Panel2 = C3 30 39 51
        Text = C3 255 255 255; Muted = C3 164 190 214; Faint = C3 104 121 141
        Accent = C3 95 168 255; AccentHi = C3 138 196 255
        RedHi = C3 240 110 134; Border = C3 40 53 64
        ArtTop = C3 32 44 58; ArtBottom = C3 14 20 26
        CheckOff = C3 90 105 120; MenuSel = C3 45 58 72
    }
}

function Set-StylePalette([string]$id) {
    $p = $script:Palettes[$id]
    $script:cBg = $p.Bg; $script:cPanel = $p.Panel; $script:cPanel2 = $p.Panel2
    $script:cText = $p.Text; $script:cMuted = $p.Muted; $script:cFaint = $p.Faint
    $script:cAccent = $p.Accent; $script:cAccentHi = $p.AccentHi
    $script:cRedHi = $p.RedHi; $script:cBorder = $p.Border
    $script:cArtTop = $p.ArtTop; $script:cArtBottom = $p.ArtBottom
    $script:cCheckOff = $p.CheckOff
    if ($script:menuColors) {
        $script:menuColors.Bg = $p.Panel
        $script:menuColors.Sel = $p.MenuSel
        $script:menuColors.Bd = $p.Border
    }
}
...
$script:iconBlue = $null
if ($logo) {
    try { $script:iconPink = [System.Drawing.Icon]::FromHandle(([System.Drawing.Bitmap]$logo).GetHicon()) } catch { }
    try {
...
function Draw-Sparkle($g, [int]$x, [int]$y, [int]$r, [int]$alpha) {
    $pen = New-Object System.Drawing.Pen ((CA $alpha $cAccentHi), (S 2))
    $pen.StartCap = "Round"
    $pen.EndCap = "Round"
...
function Get-DiscordInstalls {
    ...
    foreach ($b in $branches) {
        ...
        if (Test-Path $indexJs) {
            ...
            if ($content -match "NullCord") { $stateKey = "stInstalled" }
...
$styleBtn = New-Pill "" $cPanel $cPanel2 $cMuted $fFoot (S 13) $true
...
if ($installs.Count -eq 0) {
    ...
    for ($idx = 0; $idx -lt $installs.Count; $idx++) {
        ...
        $badgeBack = $cPanel2
        $badgeFore = $cMuted
        if ($i.StateKey -eq "stInstalled") { $badgeBack = $cAccent; $badgeFore = [System.Drawing.Color]::White }
...
# --- buttons ---
$btnInstall = New-Pill "" $cAccent $cAccentHi ([System.Drawing.Color]::White) $fBtn (S 12) $false
...
# --- creator code (optional referral; written to %APPDATA%\NullCord\referral.json on install) ---
...
function Update-Language {
    ...
    $langBtn.Invalidate()
    $stKey = "stylePink"
    if ($script:StyleId -eq "blue") { $stKey = "styleBlue" }
    $styleBtn.Tag.Text = (T $stKey)
    $sw = [System.Windows.Forms.TextRenderer]::MeasureText($styleBtn.Tag.Text, $fFoot).Width + (S 24)
    $styleBtn.Width = $sw
    $styleBtn.Left = $ghBtn.Left - $sw - (S 8)
    $styleBtn.Invalidate()
    $footNote.Text = "NullCord (c) 2026   -   " + (T "toS")
...
function Update-Style {
    ...
    $footNote.ForeColor = $cFaint
    if ($script:emptyLbl) {
        $script:emptyLbl.ForeColor = $cMuted
        $script:emptyLbl.BackColor = $cPanel
        if ($script:emptyLbl.Parent) { $script:emptyLbl.Parent.Invalidate() }
    }
    $btnInstall.Tag.Back = $cAccent
    $btnInstall.Tag.Hover = $cAccentHi
    foreach ($p in @($btnRepair, $btnUninstall, $ghBtn, $btnMin, $langBtn, $styleBtn)) {
        $p.Tag.Back = $cPanel
        $p.Tag.Hover = $cPanel2
    }
    $btnClose.Tag.Back = $cPanel
    $btnUninstall.Tag.Fore = $cRedHi
    $ghBtn.Tag.Fore = $cMuted
    $btnMin.Tag.Fore = $cMuted
    $styleBtn.Tag.Fore = $cMuted
    foreach ($p in @($btnInstall, $btnRepair, $btnUninstall, $ghBtn, $btnMin, $btnClose, $langBtn, $styleBtn)) { $p.Invalidate() }
    foreach ($k in @($script:rowCb.Keys)) {
        Set-RowHover $script:rowCb[$k].Parent $false
        $script:rowCb[$k].Invalidate()
    }
    foreach ($k in @($script:rowBadge.Keys)) {
        $d = $script:rowBadge[$k]
        $back = $cPanel2
        $fore = $cMuted
        if ($d.Key -eq "stInstalled") { $back = $cAccent; $fore = [System.Drawing.Color]::White }
        elseif ($d.Key -eq "stOther") { $back = [System.Drawing.Color]::FromArgb(124, 92, 160); $fore = [System.Drawing.Color]::White }
        $d.Back = $back
        $d.Fore = $fore
    }
    $progress.Invalidate()
        ...
        if ($mode -eq "install") {
            ...
            for ($t = 0; $t -lt 2 -and -not $ok; $t++) {
                ...
                try {
                    ...
                    $req.UserAgent = "NullCord-Installer"
                    $req.Timeout = 30000
            ...
            if (-not $ok) {
                ...
                if (Test-Path $curl) {
                    ...
                    $st.Pct = 0
                    $st.Note = $L.noteRetry
                    & $curl -L --fail --silent --show-error -A NullCord-Installer -o $AsarPath $AsarUrl 2>$null
                    $ok = Test-AsarFile $expected
                }
            }
            if (-not $ok) {
...
# Writes the optional creator code to %APPDATA%\NullCord\referral.json (Roaming — the same data dir
...
function Save-CreatorCode {
    try {
        ...
        if ($code -match '^[a-z0-9_-]{3,20}$') {
            $dir = Join-Path $env:APPDATA "NullCord"
