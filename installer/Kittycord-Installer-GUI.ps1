...
public static class KittycordImaging {
...
$script:StringsBlue = @{
    ...
    ru = @{
        headline1        = "Discord,"
        headline2        = "your way"
        tagline          = "Плагины, темы и полный контроль - патч прямо в ваш Discord за считанные секунды."
...
if ($logo) {
    ...
    try {
        $script:logoBlue = [KittycordImaging]::HueShift($logo, -117)
...
if ($script:darkMenuOk) {
    try {
        $script:menuColors = New-Object KittycordMenuColors
        Set-StylePalette $script:StyleId
