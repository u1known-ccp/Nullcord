...
<#
    NullCord — online installer

    Self-contained installer (no repo or pnpm needed). Downloads the latest NullCord build
    (desktop.asar) from GitHub Releases and patches your Discord desktop client to load it.
    This is the script that gets compiled into NullCord-Installer.exe by CI.

    Usage (do NOT run as Administrator):
        powershell -ExecutionPolicy Bypass -File .\installer\NullCord-Online-Install.ps1

    Run again any time to re-patch after a Discord update. Uninstall with NullCord-Uninstall.ps1.
#>
...
Write-Host "NullCord installer" -ForegroundColor Magenta

# 1. Download the latest build
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
New-Item -ItemType Directory -Force -Path $InstallDir | Out-Null
Write-Host "Downloading latest NullCord build..." -ForegroundColor Cyan
try {
...
try {
    ...
    console.error("[NullCord] Failed to load, starting vanilla Discord:", err);
    require("../_app.asar");
}
"@
...
Write-Host ""
Write-Host "NullCord installed. Start Discord again to see it." -ForegroundColor Magenta
