...
async function claim(code: string) {
    ...
    if (!Invites) { showToast("Invite codes work on the NullCord desktop app.", Toasts.Type.FAILURE); return; }
    ...
    else showToast("Couldn't reach NullCord to claim that code — try again later.", Toasts.Type.FAILURE);
}
...
export default definePlugin({
    name: "DeepLinks",
    description: "Opens NullCord:// links in the client: claim a friend's invite code or open a shared theme with one click.",
    ...
    start() {
        if (!wired) {
            VencordNative.NullCordDeepLinks.onLink(handle);
            wired = true;
        }
        VencordNative.NullCordDeepLinks.poll().then(handle);
    }
});
