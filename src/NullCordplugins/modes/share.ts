...
export function buildModeFile(mode: Mode): File {
    ...
    return new File([data], `NullCord-mode-${slug}-${moment().format("YYYY-MM-DD")}${FILE_SUFFIX}`, { type: "application/json" });
}
...
function sanitizeMode(raw: any): Mode {
    if (!raw || typeof raw !== "object" || typeof raw.name !== "string")
        throw new Error("That file is not a NullCord mode.");
...
export async function fetchMode(attachment: MessageAttachment): Promise<FetchedMode> {
    ...
    if (!obj || obj.kind !== "NullCord-mode" || typeof obj.v !== "number")
        throw new Error("That file is not a NullCord mode.");
