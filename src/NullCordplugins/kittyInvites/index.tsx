...
async function openInviteStatsShare() {
    const me = UserStore.getCurrentUser();
    if (!Native || !me) {
        showToast("Invite stats are available on the NullCord desktop app.", Toasts.Type.FAILURE);
        return;
    }
    try {
        ...
        openModal(props => (
            <ShareFileModal
                ...
                blurb="Show off how many friends you've brought to NullCord."
                buildFile={() => file}
                defaultNote={`I've invited ${mine.invites} ${mine.invites === 1 ? "friend" : "friends"} to NullCord 🐱 NullCord.dev`}
            />
        ));
    } catch {
        ...
        <ErrorBoundary noop>
            <div className="kc-inv-card">
                ...
                <Text variant="text-sm/normal" style={{ opacity: .75, margin: "4px 0 10px" }}>
                    Share it with friends — when they install NullCord and type your code, it counts for you.
                </Text>
...
export default definePlugin({
    name: "KittyInvites",
    description: "See who invited the most people to NullCord. Claim your own creator code and climb the monthly leaderboard.",
    ...
    stop() {
        removeFromArray(SettingsPlugin.customEntries, e => e.key === "NullCord_invites");
        disableStyle(style);
    }
});
