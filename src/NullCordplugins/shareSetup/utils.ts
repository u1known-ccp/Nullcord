...
export async function buildEnvelopeFile(scope: ShareScope): Promise<File> {
    ...
    return new File([data], `NullCord-setup-${moment().format("YYYY-MM-DD")}${FILE_SUFFIX}`, { type: "application/json" });
}
...
export function parseEnvelope(text: string): ShareEnvelope {
    ...
    if (!obj || obj.kind !== "NullCord-share" || typeof obj.v !== "number" || typeof obj.body !== "string")
        throw new Error("That file is not a NullCord setup.");
