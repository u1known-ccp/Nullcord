...
// Rebrands Discord's loading experience to NullCord:
//  - the small startup splash/update window, and
//  - the main client loading screen (the "Did you know..." tips screen).
// Both are covered with a NullCord overlay that removes itself once Discord is ready. The DOM is
...
// The NullCord logo as an <img> filling its rounded container (the real app icon as a data URL).
const KITTY_IMG = '<img src="' + KITTY_ICON_DATA_URL + '" alt="NullCord" style="width:100%;height:100%;display:block">';
...
(function () {
    try {
        ...
        o.innerHTML = '<div class="kc-stage"><div class="kc-halo"></div><div class="kc-logo">${KITTY_IMG}</div></div>' +
            '<div class="kc-name">NullCord</div>' +
    ...
    try {
        ...
        win.webContents.on("dom-ready", () => {
            try {
                ...
                const preset = ACCENT_PRESETS[RendererSettings.store.NullCordAccent!] ?? ACCENT_PRESETS.pink;
