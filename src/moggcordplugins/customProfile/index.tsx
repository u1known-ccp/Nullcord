...
// Ported to Kittycord and audited (clean: all customization is local — DataStore/localStorage +
// in-memory store hooks; only cdn.discordapp.com badge/decoration/avatar images are loaded for
// display. No auth-token reads, no external network requests, no eval).
...
const LS_KEY_DATA = "KittycordCP_data";
const LS_KEY_ENABLED = "KittycordCP_enabled";
const DS_ALL_DATA = "customProfile_allData";
const DS_ALL_ENABLED = "customProfile_allEnabled";
const LS_ALL_DATA = "KittycordCP_allData";
const LS_ALL_ENABLED = "KittycordCP_allEnabled";
...
// Inline replacement for moggcord's addCategorizedUserContextItem helper
function addCategorizedUserContextItem(children: any[], category: string, item: any) {
    if (!children || !Array.isArray(children)) return;

    let group = children.find((c: any) => c?.props?.id === "kittycord-user-context-group");

    if (!group) {
        const rootItem = <Menu.MenuItem id="kittycord-user-root" label="Kittycord Tools">{[]}</Menu.MenuItem>;
        group = <Menu.MenuGroup id="kittycord-user-context-group">{rootItem}</Menu.MenuGroup>;
        children.push(group);
    }

    const rootMenuItem = React.Children.only(group.props.children) as any;
    if (!Array.isArray(rootMenuItem.props.children)) {
        rootMenuItem.props.children = rootMenuItem.props.children ? [rootMenuItem.props.children] : [];
    }

    const catId = `kc-category-${category.toLowerCase().replace(/[^a-z0-9]/g, "-")}`;

    let categoryItem = rootMenuItem.props.children.find((c: any) => c?.props?.id === catId);

    if (!categoryItem) {
        categoryItem = <Menu.MenuItem id={catId} label={category}>{[]}</Menu.MenuItem>;
        rootMenuItem.props.children.push(categoryItem);
    }

    if (!Array.isArray(categoryItem.props.children)) {
        categoryItem.props.children = categoryItem.props.children ? [categoryItem.props.children] : [];
    }

    categoryItem.props.children.push(item);
}
...
function needsDomTextObserver() {
    return isEnabled && Boolean(storedData.username || storedData.globalName || storedData.createdAt);
}
...
let _realUsername = "";
let _realGlobalName = "";
let _realDateCacheKey = "";
let _realDateCache: string[] = [];
let _fakeDateCacheKey = "";
let _fakeDateCache: string[] = [];
...
let _domPatchedText = false;
const DOM_TEXT_SCAN_LIMIT = 600;

function shouldSkipDomText(node: Node) {
    const element = node.nodeType === Node.TEXT_NODE
        ? (node as Text).parentElement
        : node instanceof Element ? node : null;
    return Boolean(element?.closest("[data-slate-editor], textarea, input, [contenteditable='true'], [role='textbox']"));
}

function shouldProcessDomMutation(mutation: MutationRecord) {
    if (!needsDomTextObserver()) return false;
    if (shouldSkipDomText(mutation.target)) return false;
    if (mutation.type === "characterData") return nodeHasReplacementText(mutation.target);
    return Array.from(mutation.addedNodes).some(nodeHasReplacementText);
}

function getCachedRealDateVariants() {
    let key = "";
    try { key = UserStore.getCurrentUser()?.id ?? ""; } catch { }
    if (key === _realDateCacheKey) return _realDateCache;
    _realDateCacheKey = key;
    _realDateCache = getRealDateVariants();
    return _realDateCache;
}

function getCachedFakeDateVariants(isoDate: string) {
    if (isoDate === _fakeDateCacheKey) return _fakeDateCache;
    _fakeDateCacheKey = isoDate;
    _fakeDateCache = getFakeDateVariants(isoDate);
    return _fakeDateCache;
}

function hasReplacementText(text: string | null) {
    ...
    if (!storedData.createdAt) return false;
    return getCachedRealDateVariants().some(realDate =>
        realDate.length >= 4 && (text.includes(realDate) || text.toLowerCase().includes(realDate.toLowerCase()))
    );
}

function nodeHasReplacementText(node: Node, limit = DOM_TEXT_SCAN_LIMIT) {
    if (shouldSkipDomText(node)) return false;
    if (node.nodeType === Node.TEXT_NODE) return hasReplacementText((node as Text).nodeValue);
    if (!(node instanceof Element)) return false;
    if (node.childElementCount === 0) return hasReplacementText(node.textContent);
    ...
    while ((n = walker.nextNode())) {
        ...
        if (!shouldSkipDomText(n) && hasReplacementText((n as Text).nodeValue)) return true;
    }
    return false;
}

function scanTextNode(node: Text) {
    if (!needsDomTextObserver() || !node.nodeValue) return;
    if (shouldSkipDomText(node)) return;
    ...
    if (!hasReplacementText(val)) return;
    let result = val;
    let replaced = false;
    if (storedData.createdAt) {
        const realDates = getCachedRealDateVariants(); const fakeDates = getCachedFakeDateVariants(storedData.createdAt);
    ...
    if (replaced && result !== node.nodeValue) { if ((node as any).__cp_orig === undefined) (node as any).__cp_orig = val; node.nodeValue = result; _domPatchedText = true; }
}

function scanNode(node: Node) {
    if (shouldSkipDomText(node)) return;
    if (node.nodeType === Node.TEXT_NODE) { scanTextNode(node as Text); return; }
    if (node instanceof Element && node.childElementCount === 0 && !hasReplacementText(node.textContent)) return;
    const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT);
    let n: Node | null;
    let scanned = 0;
    while ((n = walker.nextNode())) {
        if (scanned++ >= DOM_TEXT_SCAN_LIMIT) break;
...
function processDomBatch() {
    _domQueued = false;
    if (!needsDomTextObserver()) { _domMutations = []; return; }
    ...
    for (const m of batch) {
        if (m.type === "characterData") scanTextNode(m.target as Text);
            ...
            if (nodeHasReplacementText(n)) scanNode(n);
        }
    }
}

function startDomObserver() {
    stopDomObserver(); if (!needsDomTextObserver()) return;
    ...
    domObserver = new MutationObserver(mutations => {
        ...
        const relevantMutations = mutations.filter(shouldProcessDomMutation);
        if (!relevantMutations.length) return;
        _domMutations.push(...relevantMutations);
            ...
            {hasOldName && (
                <div className="cp-field" style={{ marginTop: 6 }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 3 }}>
                        {'Ex: OldUser#5954 — will appear as "Old username: OldUser#5954" when hovering the badge.'}
                    </div>
                </div>
            )}
    ...
    return (
        <ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent className="cp-content">
                ...
                <Field label="Email address (local display)" value={data.email ?? ""} placeholder="example@mail.com" onChange={v => set("email", v)} />
                <Field label="Phone (local display)" value={data.phone ?? ""} placeholder="+1 555 000 0000" onChange={v => set("phone", v)} />
                <div className="cp-divider" />
                <BadgePicker
...
export default definePlugin({
    ...
    fakeCurrentUser(user: any) {
        ...
        Object.defineProperty(clone, "username", {
            get: () => isEnabled ? fakeUserName : realUsername,
        ...
        if (isEnabled && storedData.nitro) {
            ...
            // Plugin enabled but Nitro simulation OFF
            // Force removal of Nitro/Boost badges if requested or simulated by mistake
    ...
    async start() {
        ...
        // Native interception on the user store.
        try {
        ...
        // Interception on MultiAccountStore (for the "Switch Account" menu)
        ...
        // Patch getAvatarDecorationURL to inject our decoration only on our own user
        try {
    ...
    userProfileBadges: [
        {
            getBadges({ userId, badges: nativeBadges }: { userId: string; guildId: string; badges: ProfileBadge[]; }) {
                ...
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.STAFF)) {
                    badgeList.push({ description: "Discord Staff", iconSrc: "https://cdn.discordapp.com/badge-icons/5e74e9b61934fc1f67c65515d1f7e60d.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 2. Partner
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.PARTNER)) {
                    badgeList.push({ description: "Partner", iconSrc: "https://cdn.discordapp.com/badge-icons/3f9748e53446a137a052f3454e2de41e.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 3. Nitro
                if (hasNitroFake) {
                    badgeList.push({ description: "NITRO\nSubscribed since 10/22/21", iconSrc: NITRO_LEVELS[nl].icon, position: 0, props: { style, title: "Nitro" } } as ProfileBadge);
                }

                // 4. HypeSquad Events
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.HYPESQUAD)) {
                    badgeList.push({ description: "HypeSquad Events", iconSrc: "https://cdn.discordapp.com/badge-icons/bf01d1073931f921909045f3a39fd264.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 5. Bug Hunter 2
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.BUG_HUNTER_2)) {
                    badgeList.push({ description: "Bug Hunter Lvl 2", iconSrc: "https://cdn.discordapp.com/badge-icons/848f79194d4be5ff5f81505cbd0ce1e6.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 6. House Badges (HypeSquad Houses)
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.BALANCE)) {
                    badgeList.push({ description: "HypeSquad Balance", iconSrc: "https://cdn.discordapp.com/badge-icons/3aa41de486fa12454c3761e8e223442e.png", position: 0, props: { style } } as ProfileBadge);
                }
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.BRAVERY)) {
                    badgeList.push({ description: "HypeSquad Bravery", iconSrc: "https://cdn.discordapp.com/badge-icons/8a88d63823d8a71cd5e390baa45efa02.png", position: 0, props: { style } } as ProfileBadge);
                }
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.BRILLIANCE)) {
                    badgeList.push({ description: "HypeSquad Brilliance", iconSrc: "https://cdn.discordapp.com/badge-icons/011940fd013da3f7fb926e4a1cd2e618.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 7. Bug Hunter 1
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.BUG_HUNTER_1)) {
                    badgeList.push({ description: "Bug Hunter Lvl 1", iconSrc: "https://cdn.discordapp.com/badge-icons/2717692c7dca7289b35297368a940dd0.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 8. Developer (Verified)
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.DEV_VERIFIED)) {
                    badgeList.push({ description: "Verified Developer", iconSrc: "https://cdn.discordapp.com/badge-icons/6df5892e0f35b051f8b61eace34f4967.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 9. Former Moderator
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.MOD_ALUMNI)) {
                    badgeList.push({ description: "Former Moderator", iconSrc: "https://cdn.discordapp.com/badge-icons/fee1624003e2fee35cb398e125dc479b.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 10. Early Supporter
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.EARLY_SUPPORTER)) {
                    badgeList.push({ description: "Early Supporter", iconSrc: "https://cdn.discordapp.com/badge-icons/7060786766c9c840eb3019e725d2b358.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 11. Server Boost
                if (hasBoostFake) {
                    badgeList.push({ description: `Server Booster — ${BOOST_LABELS[bm]}`, iconSrc: BOOST_ICONS[bm], position: 0, props: { style, title: `Server Booster — ${BOOST_LABELS[bm]}` } } as ProfileBadge);
                }

                // 12. Active Developer
                if (storedData.badgeFlags && (storedData.badgeFlags & FLAG.ACTIVE_DEVELOPER)) {
                    badgeList.push({ description: "Active Developer", iconSrc: "https://cdn.discordapp.com/badge-icons/6bdc42827a38498929a4920da12695d9.png", position: 0, props: { style } } as ProfileBadge);
                }
                ...
                if (storedData.customBadgeIds?.includes("oldname")) {
                    ...
                    badgeList.push({ description: oldNameText, iconSrc: OLD_NAME_BADGE_ICON, position: 0, props: { style, title: oldNameText } } as ProfileBadge);
                }

                // 14. Completed Quest
                if (storedData.customBadgeIds?.includes("quest")) {
                    badgeList.push({ description: "Completed a quest", iconSrc: "https://cdn.discordapp.com/badge-icons/7d9ae358c8c5e118768335dbe68b4fb8.png", position: 0, props: { style } } as ProfileBadge);
                }

                // 15. Orbs
                if (storedData.customBadgeIds?.includes("orbs")) {
                    badgeList.push({ description: "Orbs — Apprentice", iconSrc: "https://cdn.discordapp.com/badge-icons/83d8a1eb09a8d64e59233eec5d4d5c2d.png", position: 0, props: { style } } as ProfileBadge);
                }
